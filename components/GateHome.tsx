 ...
 * 树影默认开启，可在「视频阴影」(LeafShadow, 新) 与「原版 SVG 枝影」(GateDapple)
 * 之间切换，风格持久化到 localStorage。
 */
...
export default function GateHome() {
  // 树影风格：默认疏影（原版 SVG 枝影），持久化到 localStorage
  const [shadowStyle, setShadowStyle] = useState<'video' | 'dapple'>('dapple')
  ...
  return (
    <div className="gate">
      ...
      {/* 树影：默认开启，视频影(新) 或 原版 SVG 枝影 */}
      {mounted && shadowStyle === 'video' && <LeafShadow shadowEnabled />}
      {mounted && shadowStyle === 'dapple' && <GateDapple />}
